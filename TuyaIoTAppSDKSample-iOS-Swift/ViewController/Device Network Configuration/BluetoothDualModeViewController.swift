...
//- (void)discoveredDeviceWithDeviceInfo:(TYBLEAdvModel *)deviceInfo mode:(DiscoveryMode)mode completion:(void(^)(DiscoveryDevice *device))completion {
//
//    NSMutableDictionary *param = [NSMutableDictionary new];
//    if (deviceInfo.productId.length > 0) [param setValue:deviceInfo.productId forKey:@"productId"];
//    if (deviceInfo.uuid.length > 0) [param setValue:deviceInfo.uuid forKey:@"uuid"];
//    if (deviceInfo.mac.length) [param setValue:deviceInfo.mac forKey:@"mac"];
//
//    DiscoveryDevice *device = [[DiscoveryDevice alloc] initWithIcon:nil name:@"New Device" deviceInfo:deviceInfo mode:mode];
//
//    [TuyaSmartRequest.new requestWithApiName:@"tuya.m.product.info.get" postData:param version:@"1.0" success:^(NSDictionary *result) {
//        NSString *name = [result objectForKey:@"name"];
//        NSString *icon = [result objectForKey:@"icon"];
//        device.name = [name isKindOfClass:NSString.class] ? name : @"New Device";
//        device.icon = [icon isKindOfClass:NSString.class] ? icon : nil;
